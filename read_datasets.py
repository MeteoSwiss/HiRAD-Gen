...
# cosmo = xr.open_zarr(COSMO_PATH)
# print(cosmo.attrs['data_request'])
...
# print(ds['air'])
# print('---------------------------------------------------------')
# temp2_era = xr.Dataset(
#         data_vars = {
#             'temperature': (["loc"],np.array(era[0,0,0,:][indeces]))
#         },
#         coords = {
#             "lat": (["loc"], np.array(era.latitudes[indeces]), {'units': 'degrees_north'}),
#             "lon": (["loc"], np.array(era.longitudes[indeces]), {'units': 'degrees_east'})
#         }
#     )

# print(temp2_era['temperature'])

# fig = plt.figure()
# ax = plt.axes(projection=ccrs.PlateCarree())
# temp2_era["temperature"].plot.pcolormesh(ax=ax)
# ax.coastlines()
# plt.savefig('era_lr.jpg')

# grid_in = xr.Dataset({
#     "lat": (["lat"], np.array(era.latitudes[indeces]), {'units': 'degrees_north'}),
#     "lon": (["lon"], np.array(era.longitudes[indeces]), {'units': 'degrees_east'})
# })

# grid_out = xr.Dataset({
#     "lat": (["lat"], np.array(cosmo.latitudes), {'units': 'degrees_north'}),
#     "lon": (["lon"], np.array(cosmo.longitudes), {'units': 'degrees_east'})
# })
...
# fig, ax = plt.subplots(subplot_kw={"projection": ccrs.PlateCarree()})
#p = ax.
# plt.scatter(x=era.longitudes[indeces][:10000], y=era.latitudes[indeces][:10000], s=1, c='blue')#c=era[0, 0, 0, :][indeces])
# ax.coastlines()
# ax.gridlines(draw_labels=True)
# plt.colorbar(p, label="K", orientation="horizontal")
# plt.savefig("temperature-2m-era.jpg")
...
# # fig, ax = plt.subplots(subplot_kw={"projection": ccrs.PlateCarree()})
# #p = ax.
# points = np.random.choice(len(cosmo.longitudes),len(cosmo.longitudes)//15)
# plt.scatter(x=cosmo.longitudes, y=cosmo.latitudes, s=1, c='red')#c=cosmo[0, 0, 0, :][::15])
# # ax.coastlines()
# # ax.gridlines(draw_labels=True)
# # plt.colorbar(p, label="K", orientation="horizontal")
# plt.savefig("temperature-2m-cosmo.jpg")
...
# ds = open_dataset(x=era, y=cosmo)

# for era_x, cosmo_y in ds:
#     print(era_x.shape, cosmo_y.shape)
#     break