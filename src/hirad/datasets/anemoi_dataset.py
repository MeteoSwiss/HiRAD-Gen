...
class AnemoiDataset(DownscalingDataset):
    def __init__(self, 
        ...
        self._output_dataset = open_dataset(output_anemoi_dataset_path, select=output_channel_names)
        ...
        start_date = self._output_dataset.metadata()['start_date']
        end_date = self._output_dataset.metadata()['end_date']
        ...
        if static_channel_names:
            static_dataset = open_dataset(output_anemoi_dataset_path, select=static_channel_names, start=start_date, end=start_date)
            ...
            self.static_mean = static_data.mean(axis=(1,2))
            self.static_std = static_data.std(axis=(1,2))
            target_shape = static_dataset.field_shape
            self.static_data_normalized = self.normalize_input(np.flip(static_data.squeeze().reshape(-1, *target_shape), 1))
        else:
        ...
        assert len(transform_channels) == len(transform_input_means) ==\
            len(transform_input_stds) == len(transform_output_means) == \
                len(transform_output_stds)
        ...
        for transform_descriptor in transform_channels:
            ...
            if transformation.startswith('box_cox'):
                ...
                if output_channel_idx is not None:
                    ...
                    self.output_std[output_channel_idx] = transform_output_stds[transform_descriptor]
            else:
    ...
    def __getitem__(self, idx):
        ...
        if 'tp' in self.input_channel_names:
            tp_idx = self.input_channel_names.index('tp')
