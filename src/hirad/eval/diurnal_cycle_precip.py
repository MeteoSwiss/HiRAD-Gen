...
LOG_INTERVAL = 1    # Log progress every N timesteps
...
def main(cfg: DictConfig):
    ...
    wet_cycle = {
        mode: [np.mean(wet_stats[mode][h]) for h in sorted(wet_stats[mode])]
        for mode in ['target','baseline']
    }
    ...
    save_plot(hrs_c, wet_lines * 100., ['COSMO-2','ERA5','Pred Mean ± Std'], 'Wet-Hour Fraction [%]',
