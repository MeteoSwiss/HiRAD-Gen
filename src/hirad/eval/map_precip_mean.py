"""
Plots maps of the mean precipitation over the entire time period.

This script computes the temporal mean of precipitation for each grid point
...
def main(cfg: DictConfig):
    ...
    logger.info("Starting computation for mean precipitation maps")
    ...
    target_mean = target_da.mean(dim='time')
    logger.info("Target mean computed")
    ...
    # Apply scaling factor for baseline and compute mean
    baseline_mean = (baseline_da / 6.0).mean(dim='time')
    logger.info("Baseline mean computed")
    ...
    # Compute mean across time for each member, then ensemble mean
    pred_mean_by_member = pred_da.mean(dim='time')
    pred_mean_mean = pred_mean_by_member.mean(dim='member')
    logger.info("Predictions mean computed")

    # Create output directory
    map_output_dir = out_root / 'maps_mean'
    ...
    plot_map_precipitation(
        target_mean.values,
        str(map_output_dir / 'target_mean'),
        title='COSMO-2 Analysis: Mean Precipitation',
        threshold=0.01,  # Lower threshold for mean precipitation
    ...
    plot_map_precipitation(
        ...
        str(map_output_dir / 'baseline_mean'),
        title='ERA5: Mean Precipitation',
        threshold=0.01,  # Lower threshold for mean precipitation
        rfac=1.0  # Already converted to mm/day
    )
    ...
    plot_map_precipitation(
        pred_mean_mean.values,
        str(map_output_dir / 'prediction_ensmean_mean'),
        title='CorrDiff Ensemble Mean: Mean Precipitation',
        threshold=0.01,  # Lower threshold for mean precipitation
        rfac=1.0  # Already converted to mm/day
    )
    ...
    n_members = pred_mean_by_member.shape[0]
    for member_idx in range(n_members):
        plot_map_precipitation(
            pred_mean_by_member[member_idx].values,
            str(map_output_dir / f'prediction_member_{member_idx:02d}_mean'),
            title=f'CorrDiff Member {member_idx+1}: Mean Precipitation',
            threshold=0.01,  # Lower threshold for mean precipitation
            rfac=1.0  # Already converted to mm/day
        )
    ...
    logger.info("Mean precipitation mapping completed successfully")
