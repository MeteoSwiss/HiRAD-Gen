...
class ResidualEnergyLoss:
    """
    Mixture loss function for denoising score matching.

    This class implements a loss function that combines deterministic
    regression with denoising score matching. It uses a pre-trained regression
    network to compute residuals before applying the diffusion process. It applies
    CRPS based energy loss on the pairs of outputs and targets from the same input
    conditioning and different noise realizations.
    ...
    def get_noise_params(self, y: torch.Tensor) -> torch.Tensor:
        ...
        """
        # Sample noise level
        rnd_normal = torch.randn([y.shape[0]//2, 1, 1, 1], device=y.device)
        sigma = (rnd_normal * self.P_std + self.P_mean).exp()
        sigma = torch.cat([sigma, sigma], dim=1).reshape(2 * sigma.shape[0], *sigma.shape[1:])
        # Loss weight
        weight = (sigma**2 + self.sigma_data**2) / (sigma * self.sigma_data) ** 2
        # Sample noise
        n = torch.randn_like(y) * sigma
    ...
    def calculate_grad(self, y: torch.tensor) -> torch.tensor :
        y_x_grad = torch.abs(y[..., 1:] - y[..., :-1])
        y_x_grad = torch.cat((y_x_grad[..., 0:1], 0.5*(y_x_grad[..., 1:]+y_x_grad[..., :-1]), y_x_grad[..., -1:]), dim=-1)
        y_y_grad = torch.abs(y[..., 1:, :] - y[..., :-1, :])
        y_y_grad = torch.cat((y_y_grad[..., 0:1, :], 0.5*(y_y_grad[..., 1:, :]+y_y_grad[..., :-1, :]), y_y_grad[..., -1:, :]), dim=-2)
        return y_x_grad, y_y_grad
    ...
    def __call__(
        ...
        """
        Calculate and return the energy loss for denoising score matching.

        This method computes a residual loss as in ~losses.loss.ResidualLoss, but applies
        a CRPS based energy loss on the pairs of outputs and targets from the same input
        conditioning and different noise realizations. This is achieved by duplicating
        the batch along the batch dimension, and applying the diffusion model to each
        half of the batch with different noise realizations. The loss is then computed
        as the CRPS based energy loss between the two halves of the batch.

        Note: this loss function applies full reduction to a single value.
        ...
        # duplicate batch for energy score
        y1 = y.clone().detach().requires_grad_(True)
        y2 = y.clone().detach().requires_grad_(True)
        y = torch.stack([y1, y2], dim=1).reshape(2 * y1.shape[0], *y1.shape[1:])
        y_lr = torch.stack([y_lr, y_lr], dim=1).reshape(2 * y_lr.shape[0], *y_lr.shape[1:])
        augment_labels = (
            torch.stack([augment_labels, augment_labels], dim=1)
            .reshape(2 * augment_labels.shape[0], *augment_labels.shape[1:])
            if augment_labels is not None
            else None
        )
        global_index = (
            torch.stack(
                [patching.global_index(batch_size, img_clean.device)] * 2, dim=1
            )
        ...
        # print(augment_labels.shape if augment_labels is not None else None)
        # print(global_index.shape if global_index is not None else None)
        # print(n.shape, sigma.shape, weight.shape)
        # print(y[:, :2, :2, :2])
        # print(y_lr[:, :2, :2, :2])
        # print(n[:, :2, :2, :2])
        # print(sigma)
        ...
        # loss = weight * ((D_yn - y) ** 2)
        # print(loss.sum())
        # print((loss.sum(dim=(1,2,3))/weight.squeeze()).sum())

        # gradient based weights for energy loss
        y_x_grad, y_y_grad = self.calculate_grad(y[0::2])
        grad_mag = torch.sqrt(y_x_grad**2 + y_y_grad**2)
        w_grad = torch.exp(-grad_mag/0.3) # /(grad_mag.mean(dim=(0,-2,-1),keepdim=True)+1e-6)

        # print("MIN MAX GRAD WEIGHTS")
        # print(torch.amin(w_grad,dim=(-2,-1)))
        # print(torch.amax(w_grad,dim=(-2,-1)))

        D_yn1, D_yn2 = D_yn[0::2], D_yn[1::2]
        spread_loss = weight[0::2]*(1-w_grad)*(D_yn1 - D_yn2).pow(2)
        precision_loss = weight*(D_yn - y).pow(2)
        precision_loss_original = precision_loss.clone().sum()/2
        precision_loss = precision_loss.sum(dim=(1,2,3)).sqrt().view(precision_loss.shape[0]//2, 2).mean(dim=1).sum()
        spread_loss = spread_loss.sum(dim=(1,2,3)).sqrt().sum()
        # loss = precision_loss.mean() - 0.5 * spread_loss.mean()
        # loss_2 = (precision_loss.mean(dim=(1,2,3)).view(precision_loss.shape[0]//2, 2).mean(dim=1) - 0.5 * spread_loss.mean(dim=(1,2,3))).mean()
        # loss = precision_loss.sum(dim=(1,2,3)).view(precision_loss.shape[0]//2, 2).mean(dim=1).sum() - 0.5 * spread_loss.sum()
        loss = precision_loss - 0.1 * spread_loss
        # print(spread_loss.sum())
        # print(precision_loss.sum())
        # print(loss)
        # print(loss_2)
        # print(loss_3)
        # print(loss_3/(spread_loss.shape[0]*spread_loss.shape[1]*spread_loss.shape[2]*spread_loss.shape[3]))

        # raise NotImplementedError("Debugging print statements present.")

        return loss, precision_loss, spread_loss, precision_loss_original
