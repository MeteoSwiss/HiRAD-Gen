...
try:
    import apex.normalization as apex_normalization
    APEX_AVAILABLE = True
except ImportError:
    pass
...
class Natten2DSelfAttention(AttentionModuleBase):
    ...
    norm_layer : Literal["apex", "torch"], optional, default="torch"
        The layer normalization backend for QK norm when ``qk_norm=True``. When used inside :class:`~hirad.models.dit_layers.DiTBlock` with ``attention_backend="natten2d"``, this is set from the block's ``layernorm_backend``.
    na2d_kwargs : Dict[str, Any], optional, default=None
        Optional keyword arguments forwarded to :func:`hirad.models.dit_layers.na2d` for performance tuning (e.g. ``dilation``, ``is_causal``, ``scale``). If ``None``, an empty dict is used.
    ...
    --------
    >>> import torch
    >>> from hirad.models import Natten2DSelfAttention
    ...
    def __init__(
        ...
        if not NATTEN_AVAILABLE:
            raise ImportError(
                "NATTEN is not installed. Please install it with `pip install NATTEN`."
            )
...
def na2d(
    ...
    **kwargs : Any
    ...
    return natten.functional.na2d(q, k, v, kernel_size, dilation=dilation, **kwargs)
...
class DiTBlock(nn.Module):
    ...
    attention_backend : Literal["timm", "transformer_engine", "natten2d"] or Module
        ...
        :class:`~hirad.models.dit_layers.TimmSelfAttention`), ``"transformer_engine"``
        (see :class:`~hirad.models.dit_layers.TESelfAttention`), ``"natten2d"``
        (see :class:`~hirad.models.dit_layers.Natten2DSelfAttention`). The expected
        interface is :class:`~hirad.models.dit_layers.AttentionModuleBase`. Default ``"timm"``.
    ...
    intermediate_dropout : bool, optional, default=False
        Whether to apply :class:`~hirad.models.dit_layers.PerSampleDropout` before attention.
    attn_drop_rate : float, optional, default=0.0
        The dropout rate for the attention operation.
    proj_drop_rate : float, optional, default=0.0
        The dropout rate for the projection operation.
    ...
    **attn_kwargs : Any
    ...
    def __init__(
        ...
        attention_backend: Union[
            Literal["transformer_engine", "timm", "natten2d"], nn.Module
        ] = "timm",
...
class DropPath(torch.nn.Module):
    ...
    def __init__(self, drop_prob: float = 0.0, scale_by_keep: bool = False):
